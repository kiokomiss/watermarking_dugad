...
void init_dwt(int cols, int rows, const char *filter_name, int filter, int level, int method) {
  ...
  dwt_cols = cols;
  dwt_rows = rows;
  printf("\n init_dwt(): dwt_filter %d\n \tdwt_method %d\n \tdwt_cols %d\n \tdwt_rows %d\n \tdwt_levels %d\n", dwt_filter, dwt_method, dwt_cols, dwt_rows, dwt_levels);
}
...
Image_tree fdwt(gray *pixels) {
  ...
  for (i = 0; i < dwt_rows; i++) 
		  ...
		  set_pixel(image, j, i, pixels[j + (i*(image->width))]);
  }
// for (i=0; i<(dwt_cols*dwt_rows);i++) (image->data[i]) = (Pixel) pixels[i];
